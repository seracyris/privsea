...
const Navbar = () => {
  const { user, setUser, setLoading, fetchUserDetails, login, logout } = useAuth(); // Use the correct functions
  ...
  const handleLoginPopup = () => setLoginPopup(true);
  const handleCreatePopup = () => setCreatePopup(true);
      ...
      if (storedUser) {
        ...
        fetchUserDetails()
          ...
          .catch(error => {
            ...
            localStorage.removeItem('token');
            localStorage.removeItem('user'); // Remove user data if fetching fails
          });
      }
    }
      ...
      <div className='px-2 flex justify-between items-center w-full h-full'>
        ...
        <div className='hidden md:flex pr-4'>
          {user ? (
              ...
              <Menu as='div' className='relative'>
                ...
                <Transition
                  ...
                  <MenuItems className='origin-top-right z-10 absolute right-0 mt-2 w-48 rounded-sm shadow-sm p-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none'>
                    ...
                    <MenuItem>
                      {({ active }) => (
                        <div className={classNames(active && 'bg-gray-100', 'text-gray-700 focus:bg-gray-200 block cursor-pointer rounded-sm px-4 py-2')}>Settings</div>
                      )}
                    </MenuItem>
                    <MenuItem>
                      {({ active }) => (
            ...
            <>
              <button onClick={handleLoginPopup} className='border-none bg-transparent text-black mr-4 outline outline-black rounded-lg py-3 px-5 outline-1 hover:bg-indigo-700 hover:text-white'>Sign In</button>
              <button onClick={handleCreatePopup} className='px-8 py-3 outline outline-black rounded-lg outline-1 hover:bg-indigo-700 hover:text-white'>Sign Up</button>
            </>
          )}
        </div>
      ...
      <ul className={!nav ? 'hidden' : 'absolute bg-zinc-200 w-full px-8'}>
        ...
        <div className='flex flex-col my-4'>
          {user ? (
              ...
              <Menu as='div' className='relative'>
                ...
                <Transition
                  ...
                  <MenuItems className='origin-top-right z-10 absolute right-0 mt-2 w-48 rounded-sm shadow-sm p-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none'>
                    <MenuItem>
                      {({ active }) => (
                        <div className={classNames(active && 'bg-gray-100', 'text-gray-700 focus:bg-gray-200 block cursor-pointer rounded-sm px-4 py-2')}>Your Profile</div>
                      )}
                    </MenuItem>
                    <MenuItem>
                      {({ active }) => (
            ...
            <>
              <button onClick={handleLoginPopup} className='bg-transparent text-indigo-600 px-8 py-3 mb-4'>Sign In</button>
              <button onClick={handleCreatePopup} className='px-8 py-3'>Sign Up</button>
            </>
          )}
        </div>
      </ul>
