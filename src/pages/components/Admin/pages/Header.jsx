        ...
        <div className='bg-white h-16 px-4 flex justify-between items-center border-b border-gray-200'>
            ...
            <div className='flex items-center gap-2 mr-2'>
                <Popover className='relative'>
                    {({ open }) => (
                        <>
                            <PopoverButton className={classNames(open && 'bg-gray-100', 'p-1.5 rounded-xl inline-flex items-center text-gray-700 hover:text-opacity-100 border-none focus:outline-none active:bg-gray-100')}>
                ...
                <Menu as='div' className='relative'>
                    ...
                    <Transition
                        ...
                        <MenuItems className='origin-top-right z-10 absolute right-0 mt-2 w-48 rounded-sm shadow-sm p-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none'>
                            <MenuItem>
                                {({ active }) => (
                                    <div className={classNames(active && 'bg-gray-100', 'text-gray-700 focus:bg-gray-200 block cursor-pointer rounded-sm px-4 py-2')} onClick={() => navigate('/profile')}>Your Profile</div>
                                )}
                            </MenuItem>
                            <MenuItem>
                                {({ active }) => (
                                    <div className={classNames(active && 'bg-gray-100', 'text-gray-700 focus:bg-gray-200 block cursor-pointer rounded-sm px-4 py-2')} onClick={() => navigate('/admin/settings')}>Settings</div>
                                )}
                            </MenuItem>
                            <MenuItem>
                                {({ active }) => (
                                    <div className={classNames(active && 'bg-gray-100', 'text-gray-700 focus:bg-gray-200 block cursor-pointer rounded-sm px-4 py-2')} onClick={handleLogout}>Sign Out</div>
                                )}
                            </MenuItem>
                        </MenuItems>
                    </Transition>
