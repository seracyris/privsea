            ...
            <nav className="h-full flex flex-col border-r shadow-sm">
                <div className="p-4 pb-2 flex justify-between items-center">
                    <div className={`overflow-hidden transition-all text-white text-2xl ${expanded ? "w-32" : "w-0"}`}>
                        {userInfo ? `${userInfo.username.charAt(0).toUpperCase() + userInfo.username.slice(1)}'s Dashboard` : ''}
                    </div>
                        ...
                        <>
                            ...
                            <div className={`flex justify-between items-center overflow-hidden transition-all ${expanded ? "w-52 ml-3" : "w-0"}`}>
                                <div className='leading-4'>
                                    <h4 className='font-semibold text-white'>{userInfo.username.charAt(0).toUpperCase() + userInfo.username.slice(1)}</h4>
...
export function SideBarItem({ onClick, icon, text, active, alert }) {
    ...
    return (
        <li onClick={onClick} className={`text-slate-200 relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors group ${active ? "bg-gradient-to-tr from-indigo-200 to-indigo-100 text-neutral-700" : "hover:bg-neutral-500 text-gray-200"}`}>
            {icon}
